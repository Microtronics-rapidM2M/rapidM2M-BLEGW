 ...
 * 
 * Reads the last valid values for Temp and RH periodically (record interval) from the system and stores the 
 ...
 *       it is necessary to define a Data Descriptor (see 30_System_Values_3.txt)
 * 
...
#include ".\rapidM2M BLEGW\blegw.inc"
...
const
{
  ...
  SIZE_RECDATA    = 1+4+4,                  // Measurement data block consists of: "Split-tag" (u8) + Temp (s32) + RH (u32)   
...
/* Reads the last valid values for Temp and RH from the system, compiles the data record to  
   be saved and transfers the compounded data record to the system to be recorded            */
RecordData()
{
  new aSysValues[TBLEGW_SysValue];         // Temporary memory for the internal measurement values
  new aRecData{SIZE_RECDATA};               // Temporary memory in which the data record to be saved is compiled
  
  BLEGW_GetSysValues(aSysValues);          // Reads the last valid values for Temp and RH from the system
  ...
  /* Compiles the data record to be saved in the "aRecData" temporary memory
     - The first byte (position 0 in the "aRecData" array) is set to 0 so that the server copies
       the data record into measurement data channel 0 upon receipt, as specified during the design
         of the connector
     - Temp is copied to position 1-4. Data type: s32
     - RH is copied to position 5-8.   Data type: u32                                        */   
  aRecData{0} = 0;                          // "Split-tag" 
  rM2M_Pack(aRecData, 1, aSysValues.Temp, RM2M_PACK_S32|RM2M_PACK_BE);
  rM2M_Pack(aRecData, 5, aSysValues.RH,   RM2M_PACK_U32|RM2M_PACK_BE);
  ...
  if(iCfg == CFG_BASIC)                     // If the changed configuration is the basic config ->
  {
    ...
    if(iSize >= CFG_SIZE)
    {
      ...
      if(iTmp != iTxMode)
      {
        ...
        rM2M_TxSetMode(iTxMode);                           // Sets connection type to the received value
      }
    }
  }
}
